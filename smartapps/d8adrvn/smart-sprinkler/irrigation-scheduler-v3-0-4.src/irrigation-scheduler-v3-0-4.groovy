...
definition(
    ...
    //iconX2Url: "water_moisture@2x.png",
...
preferences {
	page(name: "schedulePage", title: "Create An Irrigation Schedule", nextPage: "sprinklerPage", uninstall: true) {
        ...
        section ("Push Notifications") {
        	href "pagePushNotifications", title: "Setup which Push Nofications to recieve, if any.", description: none, 
            	//image: "https://s3.amazonaws.com/smartapp-icons/Meta/App-NotifyWhenNotHere.png"
                image: "http://s3.amazonaws.com/smartapp-icons/Meta/garage_contact.png"
        }
        section("Additional Options") {
			href "pageTestSchedule", title: "Test This Scheule", description: none, 
            	image: "https://raw.githubusercontent.com/MichaelStruck/SmartThingsPublic/master/img/settings.png"
        }
    }
    page(name: "pagePushNotifications", title: "Choose Push Notifications", uninstall: false) {
    	section("Select which Push Notications to recieve. Exceptions/Errors is highly recommended.") {
            input "isRunNotificationEnabled", "boolean", title: "Send Notification When Irrigation Starts", description: "Do You Want To Receive Push Notifications?", defaultValue: "false", required: false
        	input "isNotRunNotificationEnabled", "boolean", title: "Send Notification When Irrigation Doesn't Start", description: "Do You Want To Receive Push Notifications?", defaultValue: "true", required: false
        	input "isRainGuageNotificationEnabled", "boolean", title: "Send Daily Rain Guage Report", description: "Do You Want To Receive Push Notifications?", defaultValue: "false", required: false
        	input "isExceptionsNotificationEnabled", "boolean", title: "Send Notification When Exceptions", description: "Do You Want To Receive Push Notifications?", defaultValue: "true", required: false
        }
    }
	...
	page(name: "weatherPage", title: "Virtual Weather Station Setup", install: true) {
		...
		section("Test the Virtual Rain Guage...") {
            href "pageRunningVirtualRainGuage", title: "Run Now", description: "Tap to run the Virutal Rain Guage Now."
        }
    ...
    page(name: "pageTestSchedule", title: "Test This Irrigation Schedue", uninstall: false) {
    	section("Select the Text below to Run this Schedule Sequence Now.") {
            href "pageRunningTestSchedule", title: "Run Now", description: "Tap to run this Schedule Right Now.  Overides Weather & Schedule"
        }
    }
    page(name: "pageRunningTestSchedule", title: "Irrigation Schedule Test", uninstall: false) {
        def wateringAttempts = 1
        //scheduling()
    	section("Irrigation Schedule is Running...") {
    		paragraph("This Irrigation Schedule has been sent to Sprinkler Device.    To stop the test, go to Sprinker Device and cancel.")
        }
    }
    page(name: "pageRunningVirtualRainGuage", title: "Irrigation Virtual Rain Guage", uninstall: false) {
    	section("Virtual Rain Guage is Running...") {
    		paragraph("This Irrigation Schedule has been sent to Sprinkler Device.    {wasWetYesterday()} To stop the test, go to Sprinker Device and cancel.")
        }
    }
}		
...
def scheduling() {
    ...
    if (runTestEnabled.equals("true")) {
    	def wateringAttempts = 1
    	log.info "Running Test Schedule..."
        state.runTestEnabled = "false"
...
def isWeatherDelay() { 
 ...
 //     check to see if virtual rainguage exceeds threshold
        if (rainGauge > (wetThreshold?.toFloat() ?: 0.5)) {
            if (isNotRunNotificationEnabled.equals("true")) {
                sendPush("Skipping watering today due to precipitation.  Virtual rain gauge reads ${rainGauge.round(2)} inches.")    
            }
...
def attempts() {
    ...
    if (isRunNotificationEnabled.equals("true")) {
    ...
    if(anyZoneTimes()) {
        ...
        switches.OnWithZoneTimes(zoneTimes.join(","), 99)
    } 
    else {
        switches.on()
    }
